...
public class UcManageProductImpl extends AbstractProductUc implements UcManageProduct {
  ...
  public List<ProductEto> updateProductQuantities(List<ProductQuantityEto> products) {
    ...
    List<ProductEto> updatedEntities = new ArrayList<>();
    products.forEach(product -> {
      ProductEntity productEntity = getProductRepository().find(product.getProductId());
      productEntity.setQuantity(productEntity.getQuantity() - product.getQuantity());
      updatedEntities.add(getBeanMapper().map(getProductRepository().save(productEntity), ProductEto.class));
    });

    getBeanMapper().mapList(updatedEntities, ProductEto.class);
