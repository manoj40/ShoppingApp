...
import com.devonfw.module.security.jwt.common.base.web.JwtLoginFilter;
...
public abstract class BaseWebSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  public void configure(HttpSecurity http) throws Exception {
    ...
    "/services/rest/logout", "/h2-console/**" };

    // load starters as pluggins.
    http = this.webSecurityConfigurer.configure(http);
    http.headers().frameOptions().sameOrigin();
    http.csrf().disable().httpBasic();
    http.cors().and().userDetailsService(this.userDetailsService).sessionManagement()
  ...
  @Inject
  public void configureGlobal(@SuppressWarnings("cdi-ambiguous-dependency") AuthenticationManagerBuilder auth)
      throws Exception {
